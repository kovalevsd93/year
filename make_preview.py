...
PAVEL_D = inline.asset(data.PAVEL, 760, keep_alpha=True)
LOGO_D  = inline.asset(data.LOGO, 96, keep_alpha=True)
...
html = html.replace(data.PAVEL, PAVEL_D).replace(data.LOGO, LOGO_D).replace(data.LOGO_FOOT, LOGOF_D)
