...
.topbar{position:sticky;top:0;z-index:60;background:rgba(251,250,253,.86);
  ...
  /* iOS Safari иногда «размазывает» blur у sticky-элемента при скролле —
     принудительный собственный композитный слой убирает этот артефакт */
...
.cond{position:relative;overflow:hidden;isolation:isolate;display:block;
  ...
  border-radius:var(--r-lg);clip-path:inset(0 round var(--r-lg));padding:clamp(24px,2.3vw,32px);
...
.cond-ico{position:relative;z-index:1;display:grid;place-items:center;width:48px;height:48px;
  border-radius:15px;background:rgba(255,255,255,.8);color:var(--c);
...
.hl-shot-frame{position:relative;aspect-ratio:4/3;border-radius:14px;
  clip-path:inset(0 round 14px);overflow:hidden;color:var(--c)}
...
.course{position:relative;overflow:hidden;isolation:isolate;
  ...
  border:1px solid rgba(255,255,255,.6);border-radius:var(--r-lg);clip-path:inset(0 round var(--r-lg));padding:clamp(24px,2.3vw,32px);
...
/* ------------------------------------------------------------------
   BONUSES
   ------------------------------------------------------------------ */
/* ни overflow:hidden, ни clip-path: в iOS Safari они съедают горизонтальный
   свайп внутри карточки — лента отзывов об ассистенте переставала листаться.
   Обрезать тут больше нечего: декоративное пятно убрано, а сама лента
   обрезает себя сама (overflow-x у .bonus-shots) */
...
.bonus-shots button{flex:none;width:min(540px,calc(100% - 34px));padding:0;border:1px solid var(--line);
...
.tariff{position:relative;overflow:hidden;isolation:isolate;display:flex;flex-direction:column;gap:14px;
  background:color-mix(in srgb,var(--accent-tint) 55%,white);
  border:1px solid rgba(255,255,255,.7);border-radius:var(--r-lg);
  clip-path:inset(0 round var(--r-lg));
  padding:clamp(26px,3vw,40px);box-shadow:0 18px 40px -30px rgba(28,20,60,.22)}
.tariff::before{content:"";position:absolute;top:-90px;right:-80px;width:200px;height:200px;
  border-radius:50%;background:var(--accent);filter:blur(60px);opacity:.14;z-index:0}
.tariff>*{position:relative;z-index:1}
.tariff-best{background:var(--accent);border-color:var(--accent);color:#fff}
.tariff-best::before{top:-30%;left:-10%;right:auto;width:70%;height:170%;border-radius:0;
  background:linear-gradient(120deg,rgba(255,255,255,.24),rgba(255,255,255,0) 60%);
  filter:none;opacity:1;transform:rotate(6deg)}
...
/* без backdrop-filter: в многоколоночной вёрстке iOS Safari берёт для него
   не ту область фона, и карточка в правой колонке отрисовывалась пустой */
.rev-card{break-inside:avoid;width:100%;margin:0 0 14px;padding:0;
  border:1px solid var(--line);background:var(--surface);
  border-radius:var(--r-sm);
  box-shadow:0 14px 30px -24px rgba(28,20,60,.24);
